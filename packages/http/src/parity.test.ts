...
describe("HTTP and Node agree", () => {
  ...
  it("reads a file whose name a URL would take for syntax", async () => {
    const path = "/deep/odd name #1 100%?.txt";
    expect(await (await httpFileSystem(base).file(path))?.text()).toBe("oddly named");
  });
